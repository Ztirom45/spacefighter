...
def reset_space(file): #create random stars
	...
	#Generate Date:
	#stars
	Star_Data = [[[randint(-6000,6000),randint(-6000,6000)]] for x in range(5000)]
	
	#asteroids
	Astoroids_Data = [[[randint(-5000,5000),randint(-5000,5000)],
			randint(0,360),
			randint(0,11),
			randint(5,15)/10,
	...
	#spaceships
	Spaceships_Data = [[[randint(-5000,5000), randint(-5000,5000)],
			randint(0,360),
			randint(0,11),
			3,
	...
	img_list = ["astoroid1","astoroid2","astoroid3"]
	
	#calculate rects from astoroids
	for i in Astoroids_Data:
		image = img_dict[img_list[i[4]]]
		image = pygame.transform.rotozoom(image,-i[1],i[3])
		i.append(tuple(image.get_rect()))
	
	dump([Star_Data,Astoroids_Data,Spaceships_Data],f)
...
def reset_player(file): #create random stars
	...
	fellts = 104
	dist=22*3
	a = int(fellts/10)  #  [x,y],  frame costume count (inventory) 
...
reset_player("../dat/player")
reset_space("../dat/space")
